...
@section('content')
    <div class="tournament-container">
        <header class="tournament-header">
            ...
            <p class="tournament-date" style="color : white"><strong>Thời gian: </strong>từ ngày
                {{ $tournament->start_date }} đến ngày
                {{ $tournament->end_date }}
            </p>
        ...
        <section class="schedule-section">
            ...
            <div class="match-card-container">
                @foreach ($tournament->matchSchedules->sortBy('match_date') as $match)
                    <div class="match-card" data-match-id="{{ $match->id }}" data-team1-name="{{ $match->team1->name }}"
                        ...
                        <div class="match-body">
                            ...
                            <p class="match-info"><strong>Sân:</strong> {{ $match->stadium->name }} -
                                Số{{ $match->field_number }}</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            toggleBtn.addEventListener('click', function () {
                teamList.classList.toggle('team-list');
                toggleBtn.textContent = teamList.classList.contains('team-list') ? 'Hiển thị danh sách đội' : 'Ẩn danh sách đội';
            });
        });
